...
export default function DailyPracticeApp() {
  ...
  const [profileDraft, setProfileDraft] = useState<ExamProfile>({ onboarded: false, dailyMinutes: 20, targets: [] });
  ...
  const activeLearningBanks = addedBanks;
  ...
  const saveProfile = async () => {
    ...
    setBusy(true);
    try {
      await api({ action: "saveExamProfile", targets: profileDraft.targets, dailyMinutes: profileDraft.dailyMinutes });
      ...
      <div className="app-frame">
        ...
        {activeModule ? renderModule() : <>
          {tab === "today" && <div className="page-content">
            <section className="goal-strip"><div className="target-summary"><span>我的报考目标 · {profile.targets.length}项</span><b>{targetSummary}</b><p>{nextExam ? `距离${nextExam.target.label}还有 ${nextExam.days} 天` : "可分别设置考试日期并开启最近一场倒计时"}</p></div><button onClick={() => { setProfileDraft(profile); setOnboardingOpen(true); }}>调整目标</button></section>
            ...
            <section className="my-banks-preview"><div className="section-heading"><div><span>我的多考试题库 · {addedBanks.length}本</span><h2>已加入的一起练，也可以单本练</h2></div><button onClick={() => setTab("banks")}>管理题库</button></div><div className="mini-bank-row">{activeLearningBanks.slice(0, 3).map((bank) => <button key={bank.code} onClick={() => void startPractice("mixed", [bank.code])}><span>{bank.subject}</span><div><b>{bank.name}</b><small>{bank.scopeLabel} · 已练 {bank.studiedCount}/{bank.questionCount}</small></div><i>开始</i></button>)}</div></section>
          ...
          {tab === "banks" && <div className="page-content subpage">
            ...
            <div className="target-scope-note"><b>综合组题范围</b><span>{addedBanks.map((bank) => bank.name).join("、") || "尚未添加题库"}</span><button onClick={() => { setProfileDraft(profile); setOnboardingOpen(true); }}>管理目标</button></div>
            ...
            <section className="bank-list">{filteredBanks.map((bank) => {
              ...
              return <article className={`bank-card bank-${bank.coverColor}`} key={bank.code}>
                <div className="bank-cover"><span>{bank.examType}</span><b>{bank.subject}</b><small>{bank.province}</small></div>
                <div className="bank-copy"><div><span>{bank.examYear ? `${bank.examYear}年` : "持续更新"} · {bank.questionCount}题</span><h3>{bank.name}</h3><p>{bank.description}</p><div className={`scope-badge ${bank.recommended ? "recommended" : bank.targetMatch ? "generic" : "mismatch"}`}>{bank.recommended ? "✓ 适合我的报考" : bank.targetMatch ? `✓ 已在目标 · ${bank.scopeLabel}` : bank.scopeLabel}</div></div>
                  <div className="bank-progress"><div><i style={{ width: `${percent}%` }} /></div><span>{percent}%</span></div>
                  <footer><small>{bank.added ? `已掌握 ${bank.masteredCount} 题 · 正在参与综合组题` : bank.targetMatch ? "与我的报考目标匹配" : bank.mismatchReason}</small><div><button disabled={busy} className={bank.added ? "remove" : ""} onClick={() => void toggleBank(bank)}>{bank.added ? "已添加" : bank.targetMatch ? "+ 添加" : "+ 添加并备考"}</button>{bank.added && <button className="start-bank" onClick={() => void startPractice("mixed", [bank.code])}>单本练</button>}</div></footer>
                </div>
              </article>;
            })}</section>
          ...
          {tab === "me" && <div className="page-content subpage">
            ...
            <div className="target-chip-row">{profile.targets.map((target) => <span className="target-chip" key={target.code}>{target.label} · {target.examYear}</span>)}</div>
          ...
          <div className="onboarding-backdrop" role="dialog" aria-modal="true" aria-label="设置备考目标">
            <section className="onboarding-card">
              <div className="onboarding-top">
                ...
                <p>国考和多个省考都可以同时选择；题库书架与报考目标分别管理。</p>
              </div>
              ...
              {profileDraft.targets.map((target) => (
                <div className="choice-group" key={target.code}>
                  <label>{target.label} · 考试年份与日期</label>
              ...
              <div className="choice-group">
                <label>每天计划学习</label>
                <div className="minute-choice">{[10, 20, 30, 45, 60].map((minutes) => <button key={minutes} className={profileDraft.dailyMinutes === minutes ? "active" : ""} onClick={() => setProfileDraft({ ...profileDraft, dailyMinutes: minutes })}>{minutes}分钟</button>)}</div>
              </div>
              <div className="province-isolation-note">报考目标用于推荐和倒计时；只有你主动加入“我的题库”的内容才会进入智能练习，可以同时混合国考和多个省考。</div>
